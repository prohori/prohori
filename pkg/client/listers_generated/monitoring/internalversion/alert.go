...
type AlertLister interface {
	...
	List(selector labels.Selector) (ret []*monitoring.Alert, err error)
...
// List lists all Alerts in the indexer.
func (s *alertLister) List(selector labels.Selector) (ret []*monitoring.Alert, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*monitoring.Alert))
	})
	return ret, err
}
...
type AlertNamespaceLister interface {
	...
	Get(name string) (*monitoring.Alert, error)
...
// List lists all Alerts in the indexer for a given namespace.
func (s alertNamespaceLister) List(selector labels.Selector) (ret []*monitoring.Alert, err error) {
...
// Get retrieves the Alert from the indexer for a given namespace and name.
func (s alertNamespaceLister) Get(name string) (*monitoring.Alert, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(monitoring.Resource("alert"), name)
	}
	return obj.(*monitoring.Alert), nil
}
