...
type CheckLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.Check, err error)
...
// List lists all Checks in the indexer.
func (s *checkLister) List(selector labels.Selector) (ret []*v1alpha1.Check, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.Check))
	})
	return ret, err
}
...
type CheckNamespaceLister interface {
	...
	Get(name string) (*v1alpha1.Check, error)
...
// List lists all Checks in the indexer for a given namespace.
func (s checkNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.Check, err error) {
...
// Get retrieves the Check from the indexer for a given namespace and name.
func (s checkNamespaceLister) Get(name string) (*v1alpha1.Check, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("check"), name)
	}
	return obj.(*v1alpha1.Check), nil
}
